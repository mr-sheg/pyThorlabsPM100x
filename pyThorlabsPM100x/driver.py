...
class ThorlabsPM100x:
            ...
            if not (addr.startswith("ASRL")):
                try:
                    ...
                    for (
                        ...
                        if (
                            model[1] in idn
                        ):  # check if idn is one of the supported models
                            if self.model_user and not (
    ...
    def move_to_next_power_range(self, direction, LastPowerRange=None):
        ...
        if not (direction == +1 or direction == -1):
            raise ValueError(
                "The input variable 'direction' must be either +1 (to increase power range) or -1 (to decrease it)."
            )
        ...
        self.TargetPowerRange = (
            (LastPowerRange * Factor)
            if (direction == +1)
            else (LastPowerRange / Factor)
        )
        ...
        self.power_range = (
            self.TargetPowerRange
        )  # Try updating the power range to the new value. The value stored in self.power_range (when retrie it) will actually be one of the valid power ranges
        # allowed by the specific powermeter.
        if (
            self.power_range == self.old_powerRange
        ):  # if after setting the desired power, the power range of the powermeters is unchanged, we call again this function
