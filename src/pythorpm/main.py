...
class interface(abstract_instrument_interface.abstract_interface):
    ...
    output = {
        "Power": 0
    }  # We define this also as class variable. This makes it possible to see which data is produced by this interface without having to create an object

    ## SIGNALS THAT WILL BE USED TO COMMUNICATE WITH THE GUI
    #                                                           | Triggered when ...                                        | Parameter(s) Sent
    #                                                       #   -----------------------------------------------------------------------------------------------------------------------
    sig_list_devices_updated = QtCore.pyqtSignal(
        list
    )  #   | List of devices is updated                                | List of devices
    sig_reading = QtCore.pyqtSignal(
        int
    )  #   | Reading status changes                                    | 1 = Started Reading, 2 = Paused Reading, 3 Stopped Reading
    sig_updated_data = QtCore.pyqtSignal(
        object
    )  #   | Data is read from instrument                              | Acquired data
    sig_wavelength = QtCore.pyqtSignal(
        int
    )  #   | Wavelength is changed                                     | Current Wavelength
    sig_min_max_wavelength = QtCore.pyqtSignal(
        int, int
    )  #   | Min and max wavelengths supported by this device are read | Current Min and max wavelengths
    sig_refreshtime = QtCore.pyqtSignal(
        float
    )  #   | Refresh time is changed                                   | Current Refresh time
    sig_power_range = QtCore.pyqtSignal(
        float
    )  #   | Power range is changed                                    | Current Power range
    sig_auto_power_range = QtCore.pyqtSignal(
        bool
    )  #   | Auto power range setting is changed                       | Current Status of auto power range (true/false)
    ##
    ...
    def __init__(self, **kwargs):
        ...
        self.settings = {"refresh_time": 0.2, "auto_power_range": True}
    ...
    def send_list_devices(self):
        if len(self.list_devices) > 0:
            list_IDNs_and_devices = [
                dev[1] + " --> " + dev[0] for dev in self.list_devices
            ]
        else:
            list_IDNs_and_devices = []
    ...
    def connect_device(self, device_full_name):
        ...
        device_name = device_full_name.split(" --> ")[
            1
        ].lstrip()  # We extract the device address from the device name
        self.logger.info(f"Connecting to device {device_name}...")
        try:
            (Msg, ID) = self.instrument.connect_device(
                device_name
            )  # Try to connect by using the method connect_device of the device driver
    ...
    def set_refresh_time(self, refresh_time):
        try:
            ...
            ):  # in this case the number in the refresh time edit box is the same as the refresh time currently stored
                return True
        except ValueError:
    ...
    def set_wavelength(self, wl):
        try:
            if int(self.instrument.wavelength) == int(
                float(wl)
            ):  # in this case the number in the refresh time edit box is the same as the wavelength currently set
                return True
            self.logger.info(
                f"Setting the wavelength to {wl} for the device {self.connected_device_name}..."
            )
    ...
    def read_wavelength(self):
        self.logger.info(
            f"Reading current wavelength from device {self.connected_device_name}..."
        )
        self.wavelength = int(self.instrument.wavelength)
        if self.wavelength == None:
            self.logger.error(
                f"An error occurred while reading the wavelength from this device."
            )
            return
        self.sig_wavelength.emit(self.instrument.wavelength)
    ...
    def read_min_max_wavelength(self):
        self.logger.info(
            f"Reading min and max wavelength supported by device {self.connected_device_name}..."
        )
        self.min_max_wls = (
            self.instrument.min_wavelength,
            self.instrument.max_wavelength,
        )
        self.sig_min_max_wavelength.emit(self.min_max_wls[0], self.min_max_wls[1])
        self.logger.info(
            f"Wavelength range: {self.min_max_wls[0]}-{self.min_max_wls[1]} nm"
        )
    ...
    def read_power_range(self):
        self.logger.info(
            f"Reading current power range from device {self.connected_device_name}..."
        )
        self.power_range = self.instrument.power_range
        if self.power_range == None:
            self.logger.error(
                f"An error occurred while reading the power range from this device."
            )
            return
    ...
    def set_auto_power_range(self, auto_power_range):
        ...
        self.logger.info(
            f"Setting the auto-ranging function to {status_string} for the device {self.connected_device_name}..."
        )
        try:
            ...
            self.logger.error(
                f"An error occurred while setting the auto-ranging status: {e}"
            )
    ...
    def read_auto_power_range(self):
        self.logger.info(
            f"Reading the status of the auto-ranging function for the device {self.connected_device_name}..."
        )
        try:
            ...
            self.sig_auto_power_range.emit(auto_power_range)
            self.logger.info(
                f"The auto-ranging function is currently set to {status_string}."
            )
            self.settings["auto_power_range"] = auto_power_range
            return auto_power_range
        except Exception as e:
            self.logger.error(
                f"An error occurred while reading the auto-ranging status: {e}"
            )
    ...
    def start_reading(self):
        ...
        self.sig_reading.emit(
            self.SIG_READING_START
        )  # This signal will be caught by the GUI
        self.continuous_read = True  # Until this variable is set to True, the function UpdatePower will be repeated continuosly
        self.logger.info(
            f"Starting reading from device {self.connected_device_name}..."
        )
    ...
    def stop_reading(self):
        ...
        self.logger.info(
            f"Stopped reading from device {self.connected_device_name}. All stored data have been deleted."
        )
        self.sig_reading.emit(
            self.SIG_READING_PAUSE
        )  # This signal will be caught by the GUI
    ...
    def update(self):
        ...
        if self.continuous_read == True:
            ...
            QtCore.QTimer.singleShot(
                int(self.settings["refresh_time"] * 1e3), self.update
            )
...
class gui(abstract_instrument_interface.abstract_gui):
    ...
    def __init__(self, interface, parent, plot=False):
        ...
        self.plot_object = (
            None  # PlotObject object of the plot where self.store_powers is plotted
        )
    ...
    def initialize(self):
        ...
        self.on_connection_status_change(
            self.interface.SIG_DISCONNECTED
        )  # When GUI is created, all widgets are set to the "Disconnected" state
        ###
    ...
    def create_widgets(self):
        ...
        self.button_StartPauseReading.setIcon(
            QtGui.QIcon(os.path.join(graphics_dir, "play.png"))
        )
        self.button_StartPauseReading.setToolTip(
            "Start or pause the reading from the powermeter. The previous data points are not discarded when pausing."
        )
        self.button_StopReading = Qt.QPushButton("")
        self.button_StopReading.setIcon(
            QtGui.QIcon(os.path.join(graphics_dir, "stop.png"))
        )
        self.button_StopReading.setToolTip(
            "Stop the reading from the powermeter. All previous data points are discarded."
        )
        self.label_RefreshTime = Qt.QLabel("Refresh time (s): ")
        self.label_RefreshTime.setToolTip(
            "Specifies how often the power is read from the powermeter (Minimum value = 0.001 s)."
        )
        self.label_RefreshTime.setAlignment(
            QtCore.Qt.AlignRight | QtCore.Qt.AlignCenter
        )
        self.edit_RefreshTime = Qt.QLineEdit()
        self.edit_RefreshTime.setToolTip(
            "Specifies how often the power is read from the powermeter (Minimum value = 0.001 s)."
        )
        ...
        widgets_row2 = [
            ...
            self.button_SetZeroPowermeter,
            self.label_RefreshTime,
            self.edit_RefreshTime,
            self.label_Power,
            self.edit_Power,
            self.button_ShowHidePlot,
        ]
        ...
        self.button_DecreasePowerRange.setToolTip(
            "Decrease the powermeter power range."
        )
        self.button_DecreasePowerRange.setMaximumWidth(25)
        self.edit_PowerRange = Qt.QLineEdit()
        self.edit_PowerRange.setToolTip(
            "Maximum power measurable in the current power range (unless 'Auto' is checked)."
        )
        ...
        self.button_IncreasePowerRange.setToolTip(
            "Increase the powermeter power range."
        )
        self.button_IncreasePowerRange.setMaximumWidth(25)
        self.box_PowerRangeAuto = Qt.QCheckBox("Auto")
        self.box_PowerRangeAuto.setToolTip(
            "Set the power range of the powermeter to Automatic."
        )
        widgets_row3 = [
            self.label_Wavelength,
            self.edit_Wavelength,
            self.label_WavelengthUnits,
            self.label_PowerRange,
    ...
    def connect_widgets_events_to_functions(self):
        self.button_RefreshDeviceList.clicked.connect(
            self.click_button_refresh_list_devices
        )
        self.button_ConnectDevice.clicked.connect(self.click_button_connect_disconnect)
        self.button_SetZeroPowermeter.clicked.connect(
            self.click_button_set_zero_powermeter
        )
        self.edit_Wavelength.returnPressed.connect(self.press_enter_wavelength)
        self.button_DecreasePowerRange.clicked.connect(
            lambda x: self.click_button_change_power_range(-1)
        )
        self.button_IncreasePowerRange.clicked.connect(
            lambda x: self.click_button_change_power_range(+1)
        )
        self.box_PowerRangeAuto.stateChanged.connect(self.click_box_PowerRangeAuto)
        self.button_StartPauseReading.clicked.connect(
            self.click_button_StartPauseReading
        )
    ...
    def on_connection_status_change(self, status):
        ...
        if status == self.interface.SIG_CONNECTED:
            ...
            if self.plot_object:
                self.plot_object.graphWidget.setLabel(
                    "left", f"Power [{self.interface.instrument._power_units}]"
                )
                self.plot_window.setWindowTitle(
                    f"Powermeter: {self.interface.connected_device_name}"
                )
    ...
    def on_reading_status_change(self, status):
        ...
        if status == self.interface.SIG_READING_START:
            self.button_StartPauseReading.setIcon(
                QtGui.QIcon(os.path.join(graphics_dir, "pause.png"))
            )
    ...
    def on_data_change(self, data):
        ...
        if self.plot_object:
            self.plot_object.data.setData(
                list(range(1, len(self.interface.stored_data) + 1)),
                self.interface.stored_data,
            )  # This line is executed even when self.continuous_read == False, to make sure that plot gets cleared when user press the stop button
    ...
    def click_button_connect_disconnect(self):
        if self.interface.instrument.connected == False:  # We attempt connection
            ...
            )  # Get the device name from the combobox
    ...
    def click_button_StartPauseReading(self):
        if self.interface.continuous_read == False:
            ...
            ):  # read the current value in the refresh_time textbox, and validates it. The function returns True/False if refresh_time was valid
                return
            if not (
                self.press_enter_wavelength()
            ):  # read the current value in the wavelength textbox, and validates it. The function returns True/False if refresh_time was valid
                return
    ...
    def create_plot(self):
        ...
        self.plot_window = (
            Qt.QWidget()
        )  # This is the widget that will contain the plot. Since it does not have a parent, the plot will be in a floating (separated) window
...
def main():
    ...
    parser.add_argument(
        "-s", "--decrease_verbose", help="Decrease verbosity.", action="store_true"
    )
    parser.add_argument(
        "-virtual", help=f"Initialize the virtual driver", action="store_true"
    )
    ...
    view = gui(
        interface=Interface, parent=window, plot=True
    )  # In this case window is the parent of the gui
    window.show()
